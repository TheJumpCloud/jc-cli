...
func (s *Server) registerUserView() {
	registerTypedAppTool(s, typedAppSpec[userViewArgs]{
		Name: "user_view",
		Description: "Show an interactive JumpCloud user profile. The response is NESTED, not flat: user{id, username, email, firstname, lastname, activated, suspended, account_locked, created}, then mfa_enrollment, groups, ssh_keys and recent_events alongside it. There is no single `status` field — it is decomposed into the activated / suspended / account_locked booleans, so test those rather than looking for a status string. Contents: header (username, email, status badges), MFA enrollment (as mfa_enrollment — deliberately NOT `mfa`, which users_get uses for a different shape, {configured, exclusion}), group memberships with resolved names, SSH keys, and recent auth events. " +
