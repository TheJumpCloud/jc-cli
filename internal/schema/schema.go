...
var Resources = map[string]ResourceSchema{
	...
	"notification-channels": {
		...
		APIVersion:    "v2",
		Verbs:         []string{"list", "get", "create", "update", "delete"},
		DefaultFields: []string{"objectId", "name", "type", "enabled", "description"},
		Fields: []FieldDef{
			{Name: "objectId", Type: "string", Description: "Unique channel identifier", ReadOnly: true},
			{Name: "name", Type: "string", Description: "Channel name", Required: true},
			{Name: "type", Type: "string", Description: "Delivery type (e.g. email, webhook)"},
			{Name: "enabled", Type: "bool", Description: "Whether the channel receives notifications"},
			{Name: "description", Type: "string", Description: "Channel description"},
			{Name: "configuration", Type: "object", Description: "Type-specific delivery settings"},
		},
		FilterSupport: true,
		SortSupport:   true,
		SortFields:    []string{"name"},
	...
	"roles": {
		...
		DefaultFields: []string{"id", "name", "description"},
		Fields: []FieldDef{
			{Name: "id", Type: "string", Description: "Unique role identifier", ReadOnly: true},
			{Name: "name", Type: "string", Description: "Role name", Required: true},
			{Name: "description", Type: "string", Description: "What the role is for"},
			{Name: "scopes", Type: "array", Description: "Permission scopes the role grants"},
			{Name: "type", Type: "string", Description: "Role type (built-in or custom)", ReadOnly: true},
		},
		FilterSupport: true,
		SortSupport:   true,
		SortFields:    []string{"name"},
	...
	"saved-views": {
		...
		APIVersion:    "v2",
		Verbs:         []string{"list", "get", "create", "update", "delete"},
		DefaultFields: []string{"id", "name", "source", "shared", "isDefault"},
		Fields: []FieldDef{
			{Name: "id", Type: "string", Description: "Unique saved view identifier", ReadOnly: true},
			{Name: "name", Type: "string", Description: "View name", Required: true},
			{Name: "source", Type: "string", Description: "Console list the view applies to"},
			{Name: "shared", Type: "bool", Description: "Whether other admins can use it"},
			{Name: "isDefault", Type: "bool", Description: "Whether it is the default view for its list"},
			{Name: "filters", Type: "object", Description: "Saved filter expressions"},
			{Name: "columns", Type: "array", Description: "Saved column selection"},
		},
		FilterSupport: true,
		SortSupport:   true,
		SortFields:    []string{"name"},
...
func BuildCommandManifest() CommandManifest {
	return CommandManifest{
		...
		Commands: []CommandEntry{
			...
			{
				Path:        "jc windows-mdm",
				Description: "Windows MDM: Policy CSP catalog + custom OMA-URI and registry policies",
				Long:        "The Windows MDM workbench: discover, author, and create custom Windows policies. `csp list/show/template` browse Microsoft's Policy CSP catalog (~230 areas, ~3,700 settings — every OMA-URI path, wire format, allowed values, OS-build applicability, ADMX-backed flag), turning a natural-language intent into the exact `{uri, format, value}` triple; catalog data is Microsoft's pinned DDF v2 snapshot, downloaded on demand from Microsoft's official URL (SHA-256-verified, cached locally, offline afterwards — never vendored, since Microsoft's terms don't permit redistribution). `oma-uri create-policy` and `registry create-policy` then POST the policy via JumpCloud's generic custom templates (`custom_oma_uri_mdm_windows` / `custom_registry_keys_policy_windows`), resolved dynamically by name — no hardcoded IDs — with all values validated up front (format/type enums with Admin Portal aliases, OMA-URI `./` path shape, hive-prefix rejection since HKEY_LOCAL_MACHINE is implied, numeric checks for int/DWORD/QWORD) and every problem aggregated into one pass. Both policy shapes are device-scoped. Supports `--plan` for safe preview. The Windows analog of `jc apple-mdm payloads`.",
				Subcommands: []string{"csp", "oma-uri", "registry"},
				Flags: []FlagEntry{
					...
					{Name: "settings-file", Type: "string", Description: "JSON file with an array of {uri, format, value} objects (oma-uri create-policy; `csp template` emits this shape)"},
					...
					{Name: "area", Type: "string", Description: "Restrict to one Policy CSP area (csp list)"},
					{Name: "search", Type: "string", Description: "Case-insensitive substring over area, name, URI, and description (csp list)"},
