...
func TestResourceNames_Sorted(t *testing.T) {
	names := ResourceNames()
	if len(names) != 20 {
		t.Fatalf("expected 20 resources, got %d", len(names))
	}
...
func TestResourceNames_ContainsAllExpected(t *testing.T) {
	expected := []string{"ad", "admins", "apple-mdm", "apps", "auth-policies", "commands", "devices", "groups", "insights", "iplists", "ldap", "org", "policies", "policy-groups", "policy-templates", "radius", "software", "system-insights", "user-states", "users"}
	names := ResourceNames()
...
func TestAllResources_Count(t *testing.T) {
	all := AllResources()
	if len(all) != 20 {
		t.Fatalf("expected 20 resources, got %d", len(all))
	}
}
...
func TestBuildCommandManifest_Structure(t *testing.T) {
	...
	if len(m.Resources) != 20 {
		t.Errorf("expected 20 resources, got %d", len(m.Resources))
	}
}
