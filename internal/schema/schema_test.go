...
func TestResourceNames_Sorted(t *testing.T) {
	names := ResourceNames()
	if len(names) != 41 {
		t.Fatalf("expected 41 resources, got %d", len(names))
	}
...
func TestAllResources_Count(t *testing.T) {
	all := AllResources()
	if len(all) != 41 {
		t.Fatalf("expected 41 resources, got %d", len(all))
	}
}
...
func TestBuildCommandManifest_Structure(t *testing.T) {
	...
	if len(m.Resources) != 41 {
		t.Errorf("expected 41 resources, got %d", len(m.Resources))
	}
}
