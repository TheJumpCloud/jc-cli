...
func TestResourceNames_Sorted(t *testing.T) {
	names := ResourceNames()
	if len(names) != 27 {
		t.Fatalf("expected 27 resources, got %d", len(names))
	}
...
func TestResourceNames_ContainsAllExpected(t *testing.T) {
	expected := []string{"ad", "admins", "app-templates", "apple-mdm", "apps", "assets", "auth-policies", "commands", "custom-emails", "devices", "duo", "groups", "gsuite", "identity-providers", "insights", "iplists", "ldap", "office365", "org", "policies", "policy-groups", "policy-templates", "radius", "software", "system-insights", "user-states", "users"}
	names := ResourceNames()
...
func TestAllResources_Count(t *testing.T) {
	all := AllResources()
	if len(all) != 27 {
		t.Fatalf("expected 27 resources, got %d", len(all))
	}
}
...
func TestBuildCommandManifest_Structure(t *testing.T) {
	...
	if len(m.Resources) != 27 {
		t.Errorf("expected 27 resources, got %d", len(m.Resources))
	}
}
