...
func TestResourceNames_Sorted(t *testing.T) {
	names := ResourceNames()
	if len(names) != 42 {
		t.Fatalf("expected 42 resources, got %d", len(names))
	}
...
func TestResourceNames_ContainsAllExpected(t *testing.T) {
	...
	// two in step.
	expected := []string{"access-requests", "ad", "admins", "alerts", "app-templates", "apple-mdm", "apps", "assets", "auth-policies", "commands", "custom-emails", "devices", "duo", "google-emm", "groups", "gsuite", "health-rule-templates", "health-rules", "identity-providers", "insights", "iplists", "ldap", "notification-channels", "office365", "org", "password-manager", "policies", "policy-groups", "policy-templates", "radius", "roles", "saas-management", "saved-views", "service-accounts", "software", "system-insights", "user-states", "users", "workday", "workflow-runs", "workflow-templates", "workflows"}
	names := ResourceNames()
...
func TestAllResources_Count(t *testing.T) {
	all := AllResources()
	if len(all) != 42 {
		t.Fatalf("expected 42 resources, got %d", len(all))
	}
}
...
func TestBuildCommandManifest_Structure(t *testing.T) {
	...
	if len(m.Resources) != 42 {
		t.Errorf("expected 42 resources, got %d", len(m.Resources))
	}
}
