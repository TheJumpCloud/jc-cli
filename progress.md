...
Beyond the PRD: 29 schema resources, 194 MCP tools, auth policy simulator, 6 security hardening fixes, interactive TUI browser with dashboard, clipboard, POST search, help overlay, export, bookmarks, and CRUD (create/edit/delete). Interactive onboarding wizard (`jc setup`). 6 TUI bug fixes (#7–#12). Insights event detail screen with AI explanation. TUI form/filter text input fixes (q/k interception, bool toggle, range copy width). Service account login 403 fix. Setup wizard 403 tolerance (#18), TUI association labels (#19), TUI form field exclusions + password (#20). TUI associations for commands/policies/policy-groups/software (#22, #23, #24), TUI table performance fix (#21). **Released v1.3.2** (2026-02-19). TUI home screen restructured to three-column grid matching JumpCloud Admin Console (KLA-151): 6 new categories, 10 placeholder items, Cloud Directories sub-menu, responsive layout, left/right column navigation. TUI `/memberof` fix for user→group and device→group associations (KLA-152). Integration test suite with 5-phase coverage. **Released v1.4.2** (2026-02-21): `--department` on `users create` + MCP `users_create` tool, `--limit` on `org list`/`apple-mdm list`/`user-states list`, `auth status --quiet` suppresses output, recipe template quoting fix (KLA-153). **Released v1.5.0** (2026-02-21): Asset Management — full CRUD for hardware assets (CLI, MCP, TUI), 5 new MCP tools, TUI placeholder promoted to active resource (KLA-154). Asset Management rewrite for real API (KLA-155): three sub-resource endpoints (`/assets/devices`, `/assets/accessories`, `/assets/locations`), dynamic nested field flattening, `ExtractNameFunc` on resolver, `--field "Label=Value"` flags, 15 MCP tools (163→173), TUI sub-menu with FlattenFunc hook. Identity Providers resource (KLA-156): V2 CRUD at `/identity-providers` with wrapped response handling (`ResponseKey` in `V2ListOptions` and `ResourceConfig`), OIDC field flattening (`oidc.clientId`/`oidc.url` → top level), 5 CLI commands with `idp` alias, 5 MCP tools (173→178), TUI with FlattenFunc, 27 schema resources. SaaS Management resource (KLA-157): V2 CRUD at `/saas-management/applications` with sub-resources (accounts, usage, licenses, catalog), 11 CLI commands with `saas` alias, 11 MCP tools (178→189), TUI promoted from placeholder, 28 schema resources. **Released v1.7.0** (2026-02-25). **Released v1.7.1** (2026-02-25): 4 bug fixes — MCP audit log `clientSecret` redaction (#25), TUI asset body wrapper (#26), TUI identity provider OIDC nesting (#27), identity-providers list `--quiet`/`--ids` suppression (#28). New `MutateBodyFunc` hook on `ResourceEntry` for resource-specific body transforms. Access Requests resource (KLA-156): V2 at `/accessrequests` for temporary elevated device privileges, 5 CLI commands (`list`/`get`/`create`/`update`/`revoke`) with `ar` alias, user/device name resolution on create, confirmation-gated revoke via POST, 5 MCP tools (189→194), TUI promoted from placeholder, 29 schema resources. Dynamic shell completions: cache-based tab-completion for resource name/ID arguments across ~95 commands in 20 resources via shared `completeResourceNames()` function and exported `resolve.ReadCacheEntries()`. Zero-latency, offline-capable. Both names and IDs offered as candidates with descriptions.
